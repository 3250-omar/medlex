// =====================================================================
// NODEMAILER (COMMENTED OUT — replaced by Resend HTTP API)
...
export async function sendPasswordResetEmail({
  ...
  const apiKey = (getEnvVar("RESEND_API_KEY") || process.env.RESEND_API_KEY)?.trim();

  const from =
    (getEnvVar("EMAIL_FROM") || process.env.EMAIL_FROM)?.trim() ||
...
<body style="margin: 0; padding: 0; background-color: #0b1522; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #e2e8f0;">
  ...
  // If Resend API key is not configured, simulate delivery in development
  if (!apiKey) {
    console.log(`\n======================================================`);
    console.log(`[MedLex Email Service] (RESEND_API_KEY not configured)`);
  ...
  try {
    ...
    if (error) {
      console.error(`[MedLex Email Service] Resend error for ${to}:`, error);
      return {
        sent: false,
        simulated: true,
        error: error.message,
    ...
    console.log(`[MedLex Email Service] Successfully sent reset code to ${to}, id: ${data?.id}`);
    return { sent: true };
  } catch (error) {
    console.error(`[MedLex Email Service] Failed to send email via Resend to ${to}:`, error);
    return {
      sent: false,
      simulated: true,
      error: error instanceof Error ? error.message : "Resend sending error",
    };
  }
}
