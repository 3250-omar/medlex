-- Course Certificate Issuance and Retrieval RPCs

-- 1. Function to retrieve the current certificate status & progress for a course
create or replace function public.get_course_certificate_status(
...
begin
  ...
  -- 1. Get enrollment for this course
  select c.id, c.title_en, e.id, e.release_id, r.version_number, e.completed_at, e.enrolled_at
    into v_course_id, v_course_title, v_enrollment_id, v_release_id, v_release_version, v_completed_at, v_enrolled_at
  ...
  select id, certificate_number, recipient_name_snapshot, course_title_snapshot, release_version_snapshot, issued_at
    into v_cert_record
  ...
  return jsonb_build_object(
    ...
    'user_name', v_user_full_name,
    'completion_date', coalesce(v_cert_record.issued_at, v_completed_at, now()),
    'has_certificate', v_cert_record.id is not null,
    'certificate', case when v_cert_record.id is not null then jsonb_build_object(
      'id', v_cert_record.id,
      'certificate_number', v_cert_record.certificate_number,
      'recipient_name', v_cert_record.recipient_name_snapshot,
      'course_title', v_cert_record.course_title_snapshot,
      'issued_at', v_cert_record.issued_at
    ) else null end
  );
end;
$$;

-- 2. Function to issue or update a course certificate
