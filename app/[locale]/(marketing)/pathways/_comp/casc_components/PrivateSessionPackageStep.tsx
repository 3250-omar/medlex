...
export function PrivateSessionPackageStep({
  offers,
  selectedOfferId,
  onSelectOffer,
  locale,
  ...
  return (
    <div className="space-y-4 py-2">
      <div className="text-sm text-char/70">
        {t("choicePackageDesc")}
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {packageOffers.map((offer) => {
          ...
          const perSessionMinor = Math.round(offer.priceMinor / offer.sessionCount);
          ...
          return (
            <button
              ...
              className={`relative flex flex-col p-5 rounded-xl border text-start transition-all min-h-[44px] cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-gold ${
                isSelected
            ...
            >
              ...
              <div className="mt-4 pt-3 border-t border-hair/60">
                ...
                <div className="text-xs text-char/60 mt-0.5">
                  {formatPrice(perSessionMinor, offer.currency)} {locale === "ar" ? "لكل جلسة" : "per session"}
                </div>
              </div>
            </button>
          );
