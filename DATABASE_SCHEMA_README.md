...
| `contacts_requests` | `id bigint generated by default as identity` PK (incremental number); `full_name`, `gmail`, `phone`, optional `professional_role`, optional `organisation`, `pathway`, optional `notes`, `locale`, `status` (pending/responded/ignored, default pending); timestamps | Inquiries and pathway interest registrations submitted through the public contact form. |
